...
import dl from "youtube-dl-exec";
...
let link: string;
const s3Key: string = `videos/${currentDate}`;
...
router.post("/youtube-link", async (req: Request, res: Response) => {
	try {
	   ...
	   const videoFilePath = await downloadAudio(link);
 
	   // Upload the MP3 file to S3
	   const key = `videos/${currentDate}/${path.basename(videoFilePath)}`;
	   await uploadToS3(videoFilePath, key);
...
const uploadToS3 = async (videoFilePath: string, key: string) => {
	const uploadParams = {
	   Bucket: bucketName,
	   Key: key,
	   Body: await fs.readFile(videoFilePath), // Read the MP3 file from the local path
	};
